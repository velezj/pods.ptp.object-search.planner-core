...
int main( int argc, char** argv )
{
  ...
  model.period_distribution.p = pow(3,8);
  model.period_distribution.q = 3 * 8;
  model.period_distribution.r = 8;
  model.period_distribution.s = 8;
  model.ruler_length_distribution.p = pow(9,4);
  model.ruler_length_distribution.q = 9 * 4;
  model.ruler_length_distribution.r = 4;
  model.ruler_length_distribution.s = 4;
  model.ruler_start_mean_distribution.dimension = dim;
  model.ruler_start_mean_distribution.means.push_back( 0.5 );
  model.ruler_start_mean_distribution.covariance = to_dense_mat( Eigen::MatrixXd::Identity(dim,dim) * (0.8*0.8) );
  ...
  // set mcmc tracing to ON, 
  // set SAMPLING tracing to ON
  process->trace_mcmc( dir );
  process->_state.trace_samples = true;
  process->_state.trace_samples_dir = dir;
