...
std::vector<nd_aabox_t> compute_empty_regions( const std::vector<nd_point_t>& points,
...
observe_cell( greedy_shortest_path_distribution_threshold_planner& planner,
...
int main( int argc, char** argv )
{
  ...
  points.push_back( point( 0.35 ) );
  points.push_back( point( 2.35 ) );
  points.push_back( point( 4.35 ) );
  points.push_back( point( 6.35 ) );
  points.push_back( point( 8.35 ) );
  ...
  model.period_distribution.p = pow(2,10);
  model.period_distribution.q = 2*10;
  model.period_distribution.r = 10;
  model.period_distribution.s = 10;
  model.ruler_length_distribution.p = pow(8,15);
  model.ruler_length_distribution.q = 8 * 15;
  model.ruler_length_distribution.r = 15;
  model.ruler_length_distribution.s = 15;
  model.ruler_start_mean_distribution.dimension = dim;
  model.ruler_start_mean_distribution.means.push_back( 0.225 );
  model.ruler_start_mean_distribution.covariance = to_dense_mat( Eigen::MatrixXd::Identity(dim,dim) * (0.5*0.5) );
  ...
  planner_params.update_model_mcmc_iterations = 20;
  ...
  sampler_planner_params.num_samples_of_point_sets = 30;
  ...
  chosen_cells.push_back( planner.visisted_grid().cell( point( 2.1 ) ) );
  chosen_regions.push_back( planner.visisted_grid().region( point( 0.1 ) ) );
  chosen_regions.push_back( planner.visisted_grid().region( point( 2.1 ) ) );
  chosen_region_negative.push_back( false );
  chosen_region_negative.push_back( false );
  planner.add_visited_cell( planner.visisted_grid().cell( point( 0.1 ) ) );
  planner.add_visited_cell( planner.visisted_grid().cell( point( 2.1 ) ) );
  planner.add_negative_observation( planner.visisted_grid().cell( point( 1.1 ) ) );
  planner.add_negative_observation( planner.visisted_grid().cell( point( 3.1 ) ) );
  planner.add_visited_cell( planner.visisted_grid().cell( point( 1.1 ) ) );
  planner.add_visited_cell( planner.visisted_grid().cell( point( 3.1 ) ) );
  {
    ...
    nd_aabox_t region = planner.visisted_grid().region( point( 0.1 ) );
    ...
    region = planner.visisted_grid().region( point( 2.1 ) );
