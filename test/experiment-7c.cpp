		       ...
		       const double epsilon = 1e-7)
{
  ...
  for( int xi = 0; xi < x_line_ticks.size() - 1; ++xi ) {
    nd_point_t xstart = x_line_ticks[xi];
    nd_point_t xend = x_line_ticks[xi+1];
    for( int yi = 0; yi < y_line_ticks.size() - 1; ++yi ) {
...
int main( int argc, char** argv )
{
  ...
  model.period_distribution.p = pow(1,40);
  model.period_distribution.q = 1*40;
  model.period_distribution.r = 40;
  model.period_distribution.s = 40;
  model.ruler_length_distribution.p = pow(3,40);
  model.ruler_length_distribution.q = 3 * 40;
  ...
  chosen_cells.push_back( planner.visisted_grid().cell( point( 1.1, 1.1 ) ) );
  chosen_regions.push_back( planner.visisted_grid().region( point( 0.1, 0.1 ) ) );
  chosen_regions.push_back( planner.visisted_grid().region( point( 1.1, 1.1 ) ) );
  ...
  planner.add_visited_cell( planner.visisted_grid().cell( point( 1.1, 1.1 ) ) );
  {
    ...
    region = planner.visisted_grid().region( point( 1.1, 1.1 ) );
    init_empty = compute_empty_regions( p, region );
    for( size_t i = 0; i < init_empty.size(); ++i ) {
      planner.add_empty_region( init_empty[i] );
    }
  }
  planner.add_negative_observation( planner.visisted_grid().cell( point( 1.1, 0.1 ) ) );
  planner.add_negative_observation( planner.visisted_grid().cell( point( 0.1, 1.1 ) ) );
  planner.add_visited_cell( planner.visisted_grid().cell( point( 1.1, 0.1 ) ) );
  planner.add_visited_cell( planner.visisted_grid().cell( point( 0.1, 1.1 ) ) );
  ...
  fout_meta << planner.visisted_grid().cell( point( 0.1, 0.1 ) ) << std::endl;
  fout_meta << planner.visisted_grid().cell( point( 1.1, 1.1 ) ) << std::endl;
  fout_meta << planner.visisted_grid().cell( point( 1.1, 0.1 ) ) << std::endl;
  fout_meta << planner.visisted_grid().cell( point( 0.1, 1.1 ) ) << std::endl;
